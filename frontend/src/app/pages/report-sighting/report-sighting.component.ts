...
import { CreateSightingRequest } from "../../core/models/sighting/create-sighting-request.model";
...
export class ReportSightingComponent implements OnInit, OnDestroy {
        ...
        navigator.geolocation.getCurrentPosition(
          ...
          (err) => {
            ...
            if (err.code === err.PERMISSION_DENIED) {
              this.error =
                "Location access was denied. Please allow it in your browser settings, or click the map to set it manually.";
            } else if (err.code === err.POSITION_UNAVAILABLE) {
              this.error =
                "Your location is currently unavailable. Click the map to set it manually.";
            } else {
              this.error =
                "Location request timed out. Click the map to set it manually.";
            }
          },
  ...
  onSubmit(): void {
    ...
    this.dogService
      .reportSighting(Number(this.id), payload, this.photoFile)
      .subscribe({
        next: () => {
        ...
        error: (err) => {
          ...
          this.error =
            err?.error?.message ??
            "Failed to report the sighting. Please try again.";
        },
      });
  }
}
